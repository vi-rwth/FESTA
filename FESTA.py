...
parser.add_argument('-md', dest='md_dir', default=os.getcwd(), help='MD-output-directory path. DEFAULT: Current directory path.', type=str)
...
parser.add_argument('-colv', dest='colvar', default='COLVAR', help='COLVAR-file in the MD-output-directory. DEFAULT: "COLVAR".', type=str)
parser.add_argument('-png', dest='fes_png', default='True', help='Specifies whether a PNG-visualization of the FES should be created. Expects True/False. DEFAULT: True.', type=str)
parser.add_argument('-nopbc', dest='nopbc', default='False', help='Suppresses the automatic periodicity search (triggered when minima touch the edges). Expects True/False. DEFAULT: False.', type=str)
parser.add_argument('-mindist', dest='mindist', default=None, help='Smallest allowed distance at which areas are considered separate minima (in CV units). DEFAULT: 2% of FES diagonal.', type=float)
...
def init_polygon(Ga,Gb,Gtolerance,Gmp_sorted_coords,Ggrouped_points, barargs):
    ...
    global a, b
    a,b, tolerance, mp_sorted_coords, grouped_points = Ga,Gb, Gtolerance, Gmp_sorted_coords, Ggrouped_points
...
def det_min_frames(j):
    ...
    try:
        ...
        pass
    indxes = []
    with tqdm.tqdm(total=len(all_points), desc=f'min {j}', position=tqdm.tqdm._get_free_pos()+j, leave=False) as pbar:
...
def printout(i):
    ...
    try:
        ...
        print(f'MDAnalysis does not support writing in {args.traj.split(".")[-1]}-format, writing in xyz-format instead')
...
def printout_pdb_cp2k(i):
    ...
    with open(f'minima/min_{i}.pdb', 'w') as minfile:
        with open(args.traj, 'r') as ftraj:
            with tqdm.tqdm(total=len(sorted_indx[i]), desc=f'writing min {i}', position=tqdm.tqdm._get_free_pos()+i, leave=False) as pbar:
                for line in ftraj:
                    try:
...
if __name__ == '__main__':
    ...
    tot_min_frames, dimX, dimY = 0, 0, 1
    ...
    for i in tqdm.tqdm(range(len(ener)),desc='collecting outline', leave=False):
        try:
            if ener[i]<args.thresh and (a_fes[i] == min_a or a_fes[i] == max_a or b_fes[i] == min_b or b_fes[i] == max_b or ener[i-1]>args.thresh or ener[i+1]>args.thresh or ener[i-dimY]>args.thresh or ener[i+dimY]>args.thresh or ener[i+1+dimY]>args.thresh or ener[i-1+dimY]>args.thresh or ener[i+1-dimY]>args.thresh or ener[i-1-dimY]>args.thresh):
    ...
    if periodicity == True:
        ...
        for elem in pbc:
            desc.append(' + '.join((f'{fes_var[pos_cvs_fes[0]+2]}: {round(np.mean(grouped_points[j], axis=0)[0],4)} {fes_var[pos_cvs_fes[1]+2]}: {round(np.mean(grouped_points[j], axis=0)[1],4)}') for j in elem))
            help_list = []
            for i in elem:
                tot_pbc.append(i)
        ...
        for i,elem in enumerate(sorted_indx):
            if not i in tot_pbc:
                desc.append(f'{fes_var[pos_cvs_fes[0]+2]}: {round(np.mean(grouped_points[i], axis=0)[0],4)} {fes_var[pos_cvs_fes[1]+2]}: {round(np.mean(grouped_points[i], axis=0)[1],4)}')
    ...
    if args.fes_png == 'True':
        ...
        plt.xticks(np.linspace(-0.5,dimX-0.5,5),np.round(np.linspace(min_a,max_a, num=5),3))
        plt.yticks(np.linspace(-0.5,dimY-0.5,5),np.round(np.linspace(max_b,min_b, num=5),3))
    ...
    with open('minima/min_overview.txt', 'w') as overviewfile:
        for i in range(len(sorted_indx)):
            if desc:
                ...
                overviewfile.writelines(f'min_{i} : {fes_var[pos_cvs_fes[0]+2]}: {round(np.mean(grouped_points[i], axis=0)[0],4)} {fes_var[pos_cvs_fes[1]+2]}: {round(np.mean(grouped_points[i], axis=0)[1],4)}\n')
    if cp2k_pdb == False:
