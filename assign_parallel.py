...
def group_numbers(numbers, max_diff):
    ...
    while any(tmplist):  
        ...
        for compare_elem in tmplist: 
            if (seed_elem[0]-compare_elem[0])**2 + (seed_elem[1]-compare_elem[1])**2 < min_distance: 
                found = True
                min_distance = (seed_elem[0]-compare_elem[0])**2 + (seed_elem[1]-compare_elem[1])**2
        ...
        if found == True and any(subgroup):
            ...
            for group in separate_groups:
                for elem in group:
                    if (seed_elem[0]-elem[0])**2 + (seed_elem[1]-elem[1])**2 < min_distance:
...
if __name__ == '__main__':
    ...
    a_fes, b_fes, ener, da, db = data_fes.T
    ...
    step, a, b, c, x_colv, y_colv, z_colv, bias, rbias, rct, work = data_colvar.T

    start1 = time.perf_counter()
    all_points, sorted_coords = [], []
    for i, elem in enumerate(a):
        all_points.append(shapely.geometry.Point(elem,b[i]))
    grouped_points = group_numbers(outline, np.sqrt(8)*tolerance)
    polygons = []
    ...
    for u,elem2 in enumerate(in_dir):
